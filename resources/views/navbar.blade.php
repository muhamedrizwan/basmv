    ...
    <div id="wrapper" class="sidenav2" ><a href="{{URL::to('/')}}/"><img class="float-right" src="assets/img/ބަސް.އެމްވީ.png"></a>
        <div class="float-right" style="width:200px; text-align:right">
            <ul class="list-unstyled float-right dv dv2" style="margin-top:50px;">
                ...
                <li class="dv dv1 nav-item{{Route::current()->uri == ('rules') ? ' active' : ''}}"><a class="nav-link dv navClass" href="{{URL::to('/')}}/rules">ގަވާއިދުތައް</a></li>
                <li class="dv dv1 nav-item{{Route::current()->uri == ('idioms') ? ' active' : ''}}"><a class="nav-link dv navClass" href="{{URL::to('/')}}/idioms">އަދަބީ ބަސް</a></li>
                <li class="dv dv1 nav-item{{Route::current()->uri == ('linguists') ? ' active' : ''}}"><a class="nav-link dv navClass" href="{{URL::to('/')}}/linguists">އަދީބުން</a></li>
                <li class="dv dv1 nav-item{{Route::current()->uri == ('dhivehiNames') ? ' active' : ''}}"><a class="nav-link dv navClass" href="{{URL::to('/')}}/dhivehiNames">ދިވެހި ނަން</a></li>
                <li class="dv dv1 nav-item{{Route::current()->uri == ('dhivehiDates') ? ' active' : ''}}"><a class="nav-link dv navClass" href="{{URL::to('/')}}/dhivehiDates">ދިވެހި ތާރީޙު</a></li>
                <li class="dv dv1 nav-item{{Route::current()->uri == ('discussion') ? ' active' : ''}}"><a class="nav-link dv navClass" href="{{URL::to('/')}}/discussion">މަޝްވަރާ ޖަގަހަ</a></li>
                @if(auth()->check() && auth()->user()->user_type == 'admin')
                <li class="nav-item dropdown">
                    <a class="nav-link dropdown-toggle dv navClass" href="#" id="navbarDropdown" role="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                        އެޑްމިން
                    </a>
                    <div class="dropdown-menu" aria-labelledby="navbarDropdown">
                        <a class="dropdown-item dv dv1 navClass" href="{{URL::to('/')}}/admin/rules">ގަވާއިދުތައް</a>
                ...
                @endif
                @if(!auth()->check())
                <li class="dv dv1 nav-item{{Route::current()->uri == ('login') ? ' active' : ''}}"><a class="nav-link dv navClass" href="{{URL::to('/')}}/login">ލޮގިން</a></li>
                @else
                <li class="dv dv1  nav-item{{Route::current()->uri == ('login') ? ' active' : ''}}"><a class="nav-link dv navClass" href="{{URL::to('/')}}/logout">ލޮގްއައުޓް</a></li>
                @endif
            </ul>
        </div>
    </div>
